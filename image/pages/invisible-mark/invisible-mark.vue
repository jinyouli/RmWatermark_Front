<template>
	<view>
		...
		<view class="main-box u-flex u-row-center">
			...
			<view :style="{width:canvas.width / 2 +'px',height:canvas.height+'px'}" class="u-abso mix-box"></view>
			<view :style="{left:canvas.width / 2 +'px',height:canvas.height+'px'}" class="mix-line u-abso"></view>
		</view>

		<!-- 底部控制区域 -->
		<view class="controller-box safe-area-inset-bottom">
			<view class="tab-box u-flex u-type-info u-type-info-light-bg">
				<view @click="openPopup" class="tab-inner tab-inner-first u-p-t-16 u-p-l-30 u-p-r-20 u-text-center">
				...
				<view @click="previewImages" class="tab-inner tab-inner-last u-p-t-16 u-p-l-20 u-p-r-30 u-text-center">
			...
			<swiper v-else class="swiper-box" :current="tabIndex" @change="swiperChange">
				...
				<!-- start 样式 -->
				<swiper-item class="u-flex-col u-row-center">
					<view class="u-p-l-44 u-p-r-44">
						<radio-group @change="sliderChange('textColor',$event)">
						<view class="u-p-b-30">
							<label class="u-flex">
								<view>
								...
								<view class="u-font-32">
									浅色图片请选我
									<view class="u-tips-color u-font-24 u-p-t-10">使用黑色颜色加深可显示水印</view>
								</view>
							</label>
						...
						<view>
							<label class="u-flex">
								...
								<view class="u-font-32">
									深色图片请选我
									<view class="u-tips-color u-font-24 u-p-t-10">使用白色颜色加深可显示水印</view>
								</view>
				...
				<swiper-item class="u-flex-col u-row-center">
					<view class="u-p-l-30 u-p-r-10">
						<view class="u-flex">
							...
							<view class="u-flex-1">
								<slider :value="setting.textRotate" @change="sliderChange('textRotate',$event)"
									min="-180" max="180" step="15" show-value block-size="18" activeColor="#303133" />
							</view>
						</view>
				...
				<swiper-item class="u-flex-col u-row-center">
					<view class="u-p-l-30 u-p-r-10">
						<view class="u-flex">
							...
							<view class="u-flex-1">
								<slider :value="setting.textDistanceX" @change="sliderChange('textDistanceX',$event)"
									min="0" max="160" step="20" show-value block-size="18" activeColor="#303133" />
							</view>
						</view>
						<view class="u-flex">
							<view class="u-font-24 u-p-l-10">
							...
							<view class="u-flex-1">
								<slider :value="setting.textDistanceY" @change="sliderChange('textDistanceY',$event)"
									min="0" max="160" step="20" show-value block-size="18" activeColor="#303133" />
							</view>
						</view>
					</view>
				...
				<swiper-item class="u-flex-col u-row-center">
					<view class="u-p-l-30 u-p-r-30">
						<view class="u-p-b-10">使用PS查看隐藏水印：</view>
						<view class="u-tips-color u-p-b-10">1.使用PS打开图片</view>
						<view class="u-tips-color u-p-b-10">2.新建黑色（浅色背景）或白色（深色背景）图层。</view>
						<view class="u-tips-color u-p-b-10">3.右键点击图层，依次选择“混合选项”-“颜色加深”，即可显示水印</view>
					</view>
				</swiper-item>
				<!-- end 说明 -->
			</swiper>
		...
		methods: {
			...
			async setCanvas() {
				...
				query.select("#canvasBox").fields({
					...
					// 输入文本 
					canvasCtx.globalAlpha = 0.006; // 首先改变透明度
					canvasCtx.font = setting.textFontSize * scaleNum + 'px bold';
					canvasCtx.fillStyle = setting.textColor; 
					// 坑 1 setFillStyle 停止维护了 2， 不再是 方法 而是参数 3 也支持rgba 
					// canvasCtx.fillStyle = "rgba(255, 255, 255, 0.3)"; // 坑 1 setFillStyle 停止维护了 2， 不再是 方法 而是参数 3 也支持rgba 

					const fontWidth = canvasCtx.measureText(setting.text).width; //获取文本宽度
					// 设定间距
					let distanceX = setting.textDistanceX * scaleNum + fontWidth;
					let distanceY = setting.textDistanceY * scaleNum;
					canvasCtx.translate(imgWidth / 2, imgHeight / 2) // 设置旋转点到中心
					let rotate = setting.textRotate
					canvasCtx.rotate(rotate * Math.PI / 180)
					let limitSize = imgHeight > imgHeight ? imgHeight : imgHeight
					for (let i = -2 * limitSize; i < limitSize; i += distanceX) {
						for (let j = -2 * limitSize; j < limitSize; j += distanceY) {
							// 填充文字，x 间距, y 间距
							canvasCtx.fillText(setting.text, i, j)
						}
					}
